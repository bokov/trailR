...
#' @export
#'
#' @return A list with elements that include...
#'
...
#'
#' @param production If `TRUE` (default) then will throw an error if it finds
#'                   uncommitted changes. Otheriwse will return
#'                   `'TEST_OUTPUT_DO_NOT_USE'` as the git stamp.
#' @param branch     If `TRUE` (default) also returns what branch is currently
#'                   checked out.
#'
#' @export
#' @return character vector of branch and git stamp or just git stamp if
#'         `branch` is `FALSE`
#'
#' Return a commit hash (for inclusion in reports for example) after first
#' making sure all changes are committed and pushed
#'
